...
<div class="content-init">
    <div class="flex content-head">
        <div class="content-title">Danh sách mã giảm giá</div>
    ...
    <div class="content-action flex">
        <div class="action-left flex">
            <div class="content-filter flex">
                <input placeholder="Nhập mã giảm giá để tìm kiếm" 
                        type="text" 
                        ref="Filter" 
                        id="filter"
                        v-model="keyword"
            ...
            <div class="filter-pop m-l-12" style="position:relative;">
                ...
                <div class="pop-filter" :class="{'show-filter':showFilter, 'h-0':!showFilter}">
                    ...
                    <div class="pop-init">
                        <div class="pop-info1">
                            <div class="m-b-3">
                                <p class="m-b-3">Loại giảm giá</p>
                                <v-combobox @getSelect="discountType=$event" 
                                            :combobox_valid="false" 
                                            :placeholder="''" 
                                            :item_text="'text'" 
                                            :items="listDiscountType" 
                                            :vmodel="discountType"
                                            :multiple="false">
                                </v-combobox>
                            </div>
                        ...
                        <div class="pop-info1">
                            <div class="m-b-3 flex">
                                <div class="w-2/5 m-r-12">
                                    <p class="m-b-3">Giảm giá</p>
                                ...
                                <div class="w-3/5">
                                    <p class="m-b-3">Giá trị</p>
                                    <div class="flex">
                                        <input type="number" v-model="valSearch" class="w-1/2 m-r-12">
                                        <input type="number" v-model="max" v-if="operation.value==2" class="w-1/2">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="pop-info3">
                            <p>Thời gian áp dụng mã giảm giá</p>
                            <div class="flex">
                                <div class="w-2/5 m-r-12">
    ...
    <discount-detail v-if="formShow" 
                    ...
                    ref="DiscountDetail"></discount-detail>
</div>
</template>
...
import { Actions, DiscountType} from '../../../../../base/vi/Resources.js';
...
export default {
    components: { CommonTable, DiscountDetail, VDatepicker, VCombobox },
    name:'DiscountAdmin',
    props:{
        loader: Boolean,
    ...
    async created() {
        try {
            ...
            this.layoutConfig = await DiscountAPI.getLayoutConfig();
            //this.dataList = await DiscountAPI.getFilterPaging(this.paramGet);
    ...
    methods:{
        ...
        async getFilterPage(){
            this.paramGet = {
                DiscountFilter: this.keyword,
                DiscountFrom: this.dateFilter.value.startDate,
                DiscountEnd: this.dateFilter.value.endDate,
                DiscountAmount: null,
                DiscountOperation: null,
                DiscountMaxAmount: null,
        ...
        async resetFilter(){
            ...
            this.discountType = this.listDiscountType[0];
            await this.getFilterPage();
            ...
            try{
                ...
                if(user){
                    ...
                    if (this.crud == CRUD.Delete) {
                        ...
                        let listDelete = this.$refs.CommonTable.listChoose.map(item => item.DiscountId);
                        console.log(listDelete);
                        var { data } = await DiscountAPI.deleteMultiObj({listEntityId: listDelete, userName: user.userName});
                        this.changeLoader(false);
                        if(data.Data){
                        ...
                        console.log(this.$refs.DiscountDetail.validateForm());
                        this.$refs.DiscountDetail.discount.UserAction = user.userName
                        if(this.$refs.DiscountDetail.validateForm() == true){
                            ...
                            if((this.crud == CRUD.Post)||(this.crud == CRUD.Copy)){
                                ...
                                // thêm loader
                                this.changeLoader(true);
                                let res = await DiscountAPI.postObj(this.$refs.DiscountDetail.discount);
                                // bỏ loader
                                this.changeLoader(false);
                                // nếu statusCode = 201
                                if(res.status == 201){
                                    ...
                                    this.$refs.CommonTable.currentPage = 1;
                                    await this.getFilterPage();
                                    if(this.$refs.DiscountDetail.post_save == true){
                                        await this.postSave();
                                    }
                                ...
                                // nếu không thêm mới thành công
                                    ...
                                    //Base.formatObj(this.$refs.DiscountDetail.discount);
                                    this.$refs.DiscountDetail.message = res.data.data[0];
                                    this.$refs.DiscountDetail.$refs.DiscountCode.classList.add("is-invalid");
                                    this.$refs.DiscountDetail.showPopup(PopupState.Duplicate);
                                }
                                // bỏ loader
                                this.changeLoader(false);
                                return;
                            ...
                            // nếu là cập nhật
                                ...
                                if(Base.checkChangeData(this.$refs.DiscountDetail.discount, this.entityDetail, this.crud, 'DiscountCode') == false){
                                    // thêm loader
                                    this.changeLoader(true);
                                    //Base.formatObjToSave(this.$refs.DiscountDetail.discount);
                                    let res = await DiscountAPI.putObj(this.$refs.DiscountDetail.discount);
                                    // bỏ loader
                                    this.changeLoader(false);
                                    // nếu statusCode = 202
                                    if(res.status == 202){
                                    ...
                                    // nếu cập nhật thất bại
                                        ...
                                        this.$refs.DiscountDetail.message = res.data.data[0];
                                        this.$refs.DiscountCode.classList.add("is-invalid");
                                        this.$refs.DiscountDetail.showPopup(PopupState.Duplicate);
                                    }
                                    this.changeLoader(false);
                                    return;
                ...
                // nếu là thêm mới hoặc nhân bản lỗi
                    ...
                    this.listMgs.push({
                        showToast: true,
                        titleMgs: Base.stringFormat(ToastMgs.PostError,[obj.DiscountName]),
                        iconToast: ToastMessageIcon.error
                    })
                }
                // nếu là cập nhật lỗi
                    ...
                    this.listMgs.push({
                        showToast: true,
                        titleMgs: Base.stringFormat(ToastMgs.PutError,[obj.DiscountName]),
                        iconToast: ToastMessageIcon.error
                    })
                }
                else {
        ...
        async postSave(){
            // reset lại validate
            this.$refs.DiscountDetail.$nextTick(() => { this.$refs.DiscountDetail.$v.$reset() })
            // đổi hành động về thêm mới
            this.changeCrud(CRUD.Post);
            // discount chuyển về giá trị ban đầu
            this.$refs.DiscountDetail.discount = {
                DiscountCode: '', 
                DiscountName:'', 
                DiscountQuestion: '', 
                DiscountAnswer: ''
            ...
            // lấy mã ncc mới
            await this.$refs.DiscountDetail.getNewCode();
            // focus vào ô input object_code
            this.$refs.DiscountDetail.$refs.DiscountCode.focus();
            // chuyển cất và thêm về false
            this.$refs.DiscountDetail.post_save = false;
        },
