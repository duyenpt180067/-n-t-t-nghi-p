...
<div class="content-init">
    <div class="flex content-head">
        <div class="content-title">Danh sách slider</div>
    ...
    <div class="content-action flex">
        <div class="action-left flex">
            <div class="content-filter flex">
                <input placeholder="Nhập chủ đề slider, mã slider để tìm kiếm" 
                        type="text" 
                        ref="Filter" 
                        id="filter"
                        v-model="keyword"
    ...
    <slider-detail  v-if="formShow" 
                    ...
                    ref="SliderDetail"></slider-detail>
</div>
</template>
...
export default {
    components: { CommonTable, SliderDetail },
    name:'SliderAdmin',
    props:{
        loader: Boolean,
    ...
    async created() {
        try {
            ...
            this.layoutConfig = await SliderAPI.getLayoutConfig();
            //this.dataList = await SliderAPI.getFilterPaging(this.paramGet);
    ...
    methods:{
        ...
        async crudObject(obj){
            try{
                ...
                if (this.crud == CRUD.Delete) {
                    ...
                    let listDelete = this.$refs.CommonTable.listChoose.map(item => item.SliderId);
                    console.log(listDelete);
                    var { data } = await SliderAPI.deleteMultiObj(listDelete);
                    this.changeLoader(false);
                    if(data.Data){
                    ...
                    if(this.$refs.SliderDetail.validateForm() == true){
                        // nếu là thêm mới hoặc nhân bản
                        if((this.crud == CRUD.Post)||(this.crud == CRUD.Copy)){
                            // thực hiện thêm mới
                            // thêm loader
                            this.changeLoader(true);
                            let res = await SliderAPI.postObj(this.$refs.SliderDetail.slider);
                            // bỏ loader
                            this.changeLoader(false);
                            // nếu statusCode = 201
                            if(res.status == 201){
                            ...
                            // nếu không thêm mới thành công
                                ...
                                //Base.formatObj(this.$refs.SliderDetail.slider);
                                this.$refs.SliderDetail.message = res.data.data[0];
                                this.$refs.SliderDetail.$refs.SliderCode.classList.add("is-invalid");
                                this.$refs.SliderDetail.showPopup(PopupState.Duplicate);
                            }
                            // bỏ loader
                            this.changeLoader(false);
                            return;
                        ...
                        // nếu là cập nhật
                            ...
                            if(Base.checkChangeData(this.$refs.SliderDetail.slider, this.entityDetail, this.crud, 'SliderCode') == false){
                                // thêm loader
                                this.changeLoader(true);
                                //Base.formatObjToSave(this.$refs.SliderDetail.slider);
                                let res = await SliderAPI.putObj(this.$refs.SliderDetail.slider);
                                // bỏ loader
                                this.changeLoader(false);
                                // nếu statusCode = 202
                                if(res.status == 202){
                                ...
                                // nếu cập nhật thất bại
                                    ...
                                    this.$refs.SliderDetail.message = res.data.data[0];
                                    this.$refs.SliderCode.classList.add("is-invalid");
                                    this.$refs.SliderDetail.showPopup(PopupState.Duplicate);
                                }
                                this.changeLoader(false);
                                return;
                ...
                // nếu là thêm mới hoặc nhân bản lỗi
                    ...
                    this.listMgs.push({
                        showToast: true,
                        titleMgs: Base.stringFormat(ToastMgs.PostError,[obj.SliderName]),
                        iconToast: ToastMessageIcon.error
                    })
                }
                // nếu là cập nhật lỗi
                    ...
                    this.listMgs.push({
                        showToast: true,
                        titleMgs: Base.stringFormat(ToastMgs.PutError,[obj.SliderName]),
                        iconToast: ToastMessageIcon.error
                    })
                }
                else {
        ...
        async postSave(){
            ...
            this.$refs.SliderDetail.slider = {};
            // lấy mã ncc mới
            await this.getNewCode();
            // focus vào ô input object_code
            this.$refs.SliderDetail.$refs.SliderCode.focus();
